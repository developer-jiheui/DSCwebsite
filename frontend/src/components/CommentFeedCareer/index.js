...
// Function component to hold comments with replies for career feed
const CommentWithReplies = ({
  ...
  // Appends the replies for a comment together
  ...
  // Variable for whether or not a comment is empty
  const [emptyComment, setEmptyComment] = useState(false);
  // Variable for holding the comment text
  const [comment, setComment] = useState("");
  // Variable for showing or hiding reply options
  ...
  // Returns the format of the comment and replies
  return (
    <Comment key={key}>
...
// Functional component for the overall comment feed for career
const CommentFeed = ({ comments, postId, refetch }) => {
  ...
  const postComment = () => {
    if (comment != "") {
      ...
      // Post the comment
      fetch(
        "http://localhost:5000/career/community/posting/career/comment/" +
          postId,
        requestOptions
      ).then((response) => {
