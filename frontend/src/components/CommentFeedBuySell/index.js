...
// Function to create a comment with replies
const CommentWithReplies = ({
  ...
  // Get all the replies from the comment
  ...
  // Variable for empty comments
  const [emptyComment, setEmptyComment] = useState(false);
  // Variable for the text for the comment
  const [comment, setComment] = useState("");
  // Variable for whether or not the comment feed is open
  ...
  // Return the top level and sub comments
  return (
    <Comment key={key}>
      ...
      <Comment.Content>
        ...
        {show && (
          ...
          >
            ...
            />
            {/* Must revise to error check in postFunction */}
            <Button
              onClick={() => {
...
// Overall component function for a comment feed of a post of a given type
const CommentFeed = ({ comments, postId, refetch }) => {
  // Variable for empty comments
  const [emptyComment, setEmptyComment] = useState(false);
  // Variable for the text of the comment
  const [comment, setComment] = useState("");

  // Function to post a new comment to the database
  const postComment = () => {
    if (comment != "") {
      ...
      // Post the comment
      fetch(
        "http://localhost:5000/buysell/community/posting/buysell/comment/" +
          postId,
        requestOptions
      ).then((response) => {
