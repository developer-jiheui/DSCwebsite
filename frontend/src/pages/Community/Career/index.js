...
const CareerPage = () => {
  const [openCreateCareerPostModal, setOpenCreateCareerPostModal] =
    useState(false);
  ...
  const loadPosts = async (word = "") => {
    ...
    const posts1 = await fetch(
      "http://localhost:5000/career/community/career/tag/",
      requestOptions
    );
  ...
  const createJobPost = (e) => {
    ...
    // Post new posting
    fetch(
      "http://localhost:5000/career/community/createcareerpost/",
      requestOptions
    ).then((response) => {
  ...
  useEffect(async () => {
    const posts = await fetch(
      "http://localhost:5000/career/community/career/",
      {
        method: "GET",
        headers: {
  ...
  const handleSearchChange = async (e) => {
    ...
    const dataset = await fetch(
      "http://localhost:5000/career/community/posting/career/q/",
      requestOptions
    );
  ...
  const sortPosts = (value, asc) => {
    if (posts != undefined) {
      ...
      if (value == "Price") {
        if (asc)
          sorted = [...posts].sort((first, second) => {
            return parseFloat(first.price) > parseFloat(second.price) ? 1 : -1;
          });
        else
          sorted = [...posts].sort((first, second) => {
            return parseFloat(first.price) < parseFloat(second.price) ? 1 : -1;
    ...
    <>
      ...
      <Container>
        <ContentContainer>
          ...
          <p className="centered-text">
            Found a cool job post? Need career advice? This is where that all
            happens!
          </p>
          <Grid columns="4">
            <Grid.Column width="4">
              <Button
                color="purple"
                onClick={() => setOpenCreateCareerPostModal(true)}
              >
                Create Post
              </Button>
            </Grid.Column>
              ...
              <Search
                ...
                onResultSelect={(e, data) => {
                  window.location.href =
                    "http://localhost:3000/community/postingcareer/" +
                    data.result._id;
                }}
                results={results}
                value={searchValue}
            ...
            <Grid.Column textAlign="right" width="4">
              <Button
                icon="filter"
                color="purple"
                onClick={flipToFilter}
              ></Button>
              <Button icon="list" color="purple" onClick={flipToSort}></Button>
              {showDropDownFilter ? (
                <DropdownFilter
                  label={["All", "Full-time", "Part-time", "Remote"]}
                  clickFunctions={[
                    () => loadPosts("All"),
                    () => loadPosts("Full-time"),
                    () => loadPosts("Part-time"),
              ...
              {showDropDownSort ? (
                <DropdownSort
                  label={["Alphabetical", "Reverse Alphabetical"]}
                  clickFunctions={[
                    () => sortPosts("Alpha", true),
                    () => sortPosts("Alpha", false),
                  ]}
                ></DropdownSort>
              ) : null}
            </Grid.Column>
      ...
      >
        ...
        <Modal.Actions>
          ...
          />
          <Button content="Post" onClick={createJobPost} positive />
        </Modal.Actions>
      </Modal>
      <Footer />
    </>
