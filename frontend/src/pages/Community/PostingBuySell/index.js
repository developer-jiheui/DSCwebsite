...
const BuyAndSell = () => {
  ...
  const postComment = (id) => {
    ...
    // Post the comment
    fetch("http://localhost:5000/buysell/community/posting/comment/" + id, requestOptions)
    .then(response => {
  ...
  useEffect(async () => {
    ...
    // Get the post with all info and comments
    const post = await fetch("http://localhost:5000/buysell/community/posting/buysell/" + id,
    {
      method: "GET",
      headers: {
          "Content-Type": "text/plain"
