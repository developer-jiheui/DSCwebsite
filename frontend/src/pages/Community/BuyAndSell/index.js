...
const BuyAndSell = () => {
  ...
  useEffect(async () => {
    const posts = await fetch(
      "http://localhost:5000/buysell/community/buyandsell/",
      {
        method: "GET",
        headers: {
  ...
  const loadPosts = async (word = "") => {
    ...
    const posts1 = await fetch(
      "http://localhost:5000/buysell/community/buysell/tag/",
      requestOptions
    );
  ...
  const handleSubmit = (e) => {
    ...
    // Post new posting
    fetch(
      "http://localhost:5000/buysell/community/createpost/",
      requestOptions
    ).then((response) => {
  ...
  const handleSearchChange = async (e) => {
    ...
    const dataset = await fetch(
      "http://localhost:5000/buysell/community/posting/buysell/q/",
      requestOptions
    );
      ...
      <Container>
        <ContentContainer>
          ...
          <p className="centered-text">
            Got stuff to sell? Looking to strike a deal?
          </p>
          <Grid columns="3" stackable={false} relaxed>
              ...
              <Search
                ...
                onResultSelect={(e, data) => {
                  window.location.href =
                    "http://localhost:3000/community/postingbuysell/" +
                    data.result._id;
                }}
                results={results}
                value={searchValue}
            ...
            <Grid.Column textAlign="right" width="4">
              ...
              {showDropDownFilter ? (
                <DropdownFilter
                  label={["All", "Selling", "Free", "Books", "Computers"]}
                  clickFunctions={[
                    () => loadPosts("All"),
                    () => loadPosts("Selling"),
                    () => loadPosts("Free"),
