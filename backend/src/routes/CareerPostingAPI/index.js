...
//   Career.findOneAndUpdate({_id: o_id}, {$set: req.body}, {upsert: false}, function(err, result) {
//     if (err) throw err;
//   });
// });
...
route.post("/community/posting/career/comment/:id", function(req, res){
  ...
  {    
    Career.findOneAndUpdate({"_id": o_id}, {$push: {"comments": doc}}, function(err, result) {
      if (err) throw err;
      res.status(200).json(true)
    });
  }
